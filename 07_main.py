...
from models import (
    ...
    SubscriptionPlan, SubscriptionFeature, SubscriptionPlanFeature, UserSubscription
)
# Temporarily disabled Google Calendar integration
...
# ============================================================================
# SUBSCRIPTION PLANS & FEATURES - Pydantic Models
# ============================================================================

class SubscriptionFeatureCreate(BaseModel):
...
class SubscriptionPlanCreate(BaseModel):
    ...
    max_health_records: Optional[int] = None
...
class SubscriptionPlanResponse(BaseModel):
    ...
    currency: str
    max_appointments_per_month: Optional[int]
...
class SubscriptionPlanWithFeaturesResponse(BaseModel):
...
async def get_dashboard_stats(
    ...
    Returns module summaries and high-level reports
    """
    try:
        ...
        for role in UserRole:
            count = db.query(User).filter(User.role == role.value).count()
            user_breakdown[role.value] = count
        
        # Appointment Statistics
        total_appointments = db.query(Appointment).count()
        appointments_today = db.query(Appointment).filter(
            func.date(Appointment.appointment_datetime) == today_start.date()
        ).count()
        ...
        for status in AppointmentStatus:
            count = db.query(Appointment).filter(Appointment.status == status.value).count()
            appointment_breakdown[status.value] = count
        
        # Next appointment for current user
        ...
        ).order_by(Appointment.appointment_datetime).first()
        
        # Therapist Statistics (using User model with therapist role)
        total_therapists = db.query(User).filter(User.role == 'therapist').count()
        active_therapists = total_therapists  # All therapists considered active for now
        
        # Health Records Statistics
        try:
            total_health_records = db.query(HealthRecord).count()
            records_today = db.query(HealthRecord).filter(
        ...
        return {
            "overview": {
                ...
                "system_health": "excellent" if today_activity > 0 else "stable"
            },
            "users": {
                "total": total_users,
            ...
            "appointments": {
                ...
                "breakdown": appointment_breakdown,
                "next_appointment": {
                    "id": next_appointment.id if next_appointment else None,
                    "datetime": next_appointment.appointment_datetime.isoformat() if next_appointment else None,
                    "status": next_appointment.status if next_appointment else None
                } if next_appointment else None
            },
            "therapists": {
                "total": total_therapists,
                "active": active_therapists,
                "availability_rate": round((active_therapists / max(total_therapists, 1)) * 100, 1)
            },
            "health_records": {
                "total": total_health_records,
                "today": records_today,
                "avg_per_user": round(total_health_records / max(total_users, 1), 1)
            },
            "nutrition": {
                "total_meals": total_meals,
                "total_nutrients": total_nutrients,
...
class MealCreate(BaseModel):
    ...
    meal_time: Optional[str] = None  # HH:MM format or time range
    period_type: Optional[str] = "Week 1"  # Week 1, Week 2, etc.
    day_number: Optional[int] = None  # Day 1-7 for weekly plans
    meal_notes: Optional[str] = None  # Special notes for this meal
    week_notes: Optional[str] = None  # Weekly guidelines/notes
...
async def create_meal(
    ...
    db_meal = Meal(
        ...
        period_type=meal.period_type or "Week 1",
        day_number=meal.day_number,
        meal_notes=meal.meal_notes,
        week_notes=meal.week_notes,
...
# ============================================================================
# SUBSCRIPTION PLANS & FEATURES ENDPOINTS
# ============================================================================
...
@app.post("/api/subscription-features", response_model=SubscriptionFeatureResponse)
async def create_subscription_feature(
    ...
    # Check if feature_code already exists
    existing = db.query(SubscriptionFeature).filter(
        SubscriptionFeature.feature_code == feature.feature_code
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="Feature code already exists")
    
    new_feature = SubscriptionFeature(**feature.dict())
    db.add(new_feature)
    db.commit()
    db.refresh(new_feature)
    
    logger.info(f"Feature created: {new_feature.feature_name} by {current_user.username}")
    return SubscriptionFeatureResponse.from_orm(new_feature)

@app.get("/api/subscription-features", response_model=List[SubscriptionFeatureResponse])
async def get_subscription_features(
    category: Optional[str] = None,
    is_active: Optional[bool] = None,
    db: Session = Depends(get_db)
):
    """Get all subscription features (Public endpoint for external platforms)"""
    query = db.query(SubscriptionFeature)
    
    if category:
        query = query.filter(SubscriptionFeature.category == category)
    if is_active is not None:
        query = query.filter(SubscriptionFeature.is_active == is_active)
    
    features = query.all()
    return [SubscriptionFeatureResponse.from_orm(f) for f in features]

@app.get("/api/subscription-features/{feature_id}", response_model=SubscriptionFeatureResponse)
async def get_subscription_feature(
    feature_id: int,
    db: Session = Depends(get_db)
):
    """Get a specific subscription feature"""
    feature = db.query(SubscriptionFeature).filter(SubscriptionFeature.id == feature_id).first()
    if not feature:
        raise HTTPException(status_code=404, detail="Feature not found")
    return SubscriptionFeatureResponse.from_orm(feature)

@app.put("/api/subscription-features/{feature_id}", response_model=SubscriptionFeatureResponse)
async def update_subscription_feature(
    feature_id: int,
    feature_update: SubscriptionFeatureUpdate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    ...
    logger.info(f"Feature updated: {feature.feature_name} by {current_user.username}")
    return SubscriptionFeatureResponse.from_orm(feature)

@app.delete("/api/subscription-features/{feature_id}")
async def delete_subscription_feature(
    ...
    logger.info(f"Feature deleted: {feature.feature_name} by {current_user.username}")
...
async def create_subscription_plan(
    ...
    logger.info(f"Plan created: {new_plan.plan_name} by {current_user.username}")
    return SubscriptionPlanResponse.from_orm(new_plan)

@app.get("/api/subscription-plans", response_model=List[SubscriptionPlanWithFeaturesResponse])
async def get_subscription_plans(
    is_active: Optional[bool] = None,
    db: Session = Depends(get_db)
):
    """Get all subscription plans with their features"""
    query = db.query(SubscriptionPlan).options(
        joinedload(SubscriptionPlan.plan_features).joinedload(SubscriptionPlanFeature.feature)
    )
    ...
    result = []
    for plan in plans:
        features = [SubscriptionFeatureResponse.from_orm(pf.feature) for pf in plan.plan_features]
        plan_dict = SubscriptionPlanResponse.from_orm(plan).dict()
        plan_dict['features'] = features
        result.append(SubscriptionPlanWithFeaturesResponse(**plan_dict))
    
    return result

@app.get("/api/subscription-plans/{plan_id}", response_model=SubscriptionPlanWithFeaturesResponse)
async def get_subscription_plan(
    plan_id: int,
    db: Session = Depends(get_db)
):
    """Get a specific subscription plan with features"""
    plan = db.query(SubscriptionPlan).filter(SubscriptionPlan.id == plan_id).first()
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
    
    # Get plan features
    plan_features = db.query(SubscriptionFeature).join(
        SubscriptionPlanFeature,
        SubscriptionPlanFeature.feature_id == SubscriptionFeature.id
    ).filter(
        SubscriptionPlanFeature.plan_id == plan_id
    ).all()
    
    return {
        **SubscriptionPlanResponse.from_orm(plan).dict(),
        "features": [SubscriptionFeatureResponse.from_orm(f) for f in plan_features]
    }
...
async def update_subscription_plan(
    ...
    logger.info(f"Plan updated: {plan.plan_name} by {current_user.username}")
...
async def delete_subscription_plan(
    ...
    logger.info(f"Plan deleted: {plan.plan_name} by {current_user.username}")
...
@app.post("/api/subscription-plans/{plan_id}/features/{feature_id}")
async def assign_feature_to_plan(
    ...
    existing = db.query(SubscriptionPlanFeature).filter(
        SubscriptionPlanFeature.plan_id == plan_id,
        SubscriptionPlanFeature.feature_id == feature_id
    ).first()
    
    if existing:
        raise HTTPException(status_code=400, detail="Feature already assigned to plan")
    
    # Create assignment
    plan_feature = SubscriptionPlanFeature(
        plan_id=plan_id,
    ...
    logger.info(f"Feature {feature.feature_name} assigned to plan {plan.plan_name} by {current_user.username}")
    return {"message": "Feature assigned to plan successfully"}

@app.delete("/api/subscription-plans/{plan_id}/features/{feature_id}")
async def remove_feature_from_plan(
    ...
    plan_feature = db.query(SubscriptionPlanFeature).filter(
        SubscriptionPlanFeature.plan_id == plan_id,
        SubscriptionPlanFeature.feature_id == feature_id
    ).first()
    
    if not plan_feature:
        raise HTTPException(status_code=404, detail="Feature assignment not found")
    
    db.delete(plan_feature)
    db.commit()
    
    logger.info(f"Feature removed from plan {plan_id} by {current_user.username}")
    return {"message": "Feature removed from plan successfully"}

@app.get("/api/subscription-plans/{plan_id}/features", response_model=List[SubscriptionFeatureResponse])
async def get_plan_features(
    ...
    features = db.query(SubscriptionFeature).join(
        SubscriptionPlanFeature,
        SubscriptionPlanFeature.feature_id == SubscriptionFeature.id
    ).filter(
        SubscriptionPlanFeature.plan_id == plan_id
...
@app.post("/api/users/{user_id}/subscribe", response_model=UserSubscriptionResponse)
async def subscribe_user_to_plan(
    ...
    """Subscribe a user to a plan (Admin only or user can subscribe themselves)"""
    # Check if admin or user subscribing themselves
    ...
    amount = plan.price_monthly if subscription.billing_cycle == 'monthly' else (plan.price_yearly or plan.price_monthly * 12)
    ...
    logger.info(f"User {user.username} subscribed to {plan.plan_name}")
    return UserSubscriptionResponse.from_orm(new_subscription)

@app.get("/api/users/{user_id}/subscription", response_model=UserSubscriptionResponse)
async def get_user_subscription(
    ...
    # Check if admin or user checking themselves
    if current_user.role != "admin" and current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Access denied")
...
@app.put("/api/users/{user_id}/subscription/cancel")
async def cancel_user_subscription(
    ...
    # Check if admin or user canceling themselves
    if current_user.role != "admin" and current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Access denied")
...
@app.get("/api/subscriptions/all", response_model=List[UserSubscriptionResponse])
async def get_all_subscriptions(
    ...
    subscriptions = query.order_by(desc(UserSubscription.created_at)).all()
    return [UserSubscriptionResponse.from_orm(s) for s in subscriptions]

# ------------------------
# Public/External API Endpoints
# ------------------------

@app.get("/api/public/subscription-plans")
async def get_public_subscription_plans(db: Session = Depends(get_db)):
    """Get active subscription plans with features (Public API for external platforms)"""
    ...
    for plan in plans:
        ...
        features = db.query(SubscriptionFeature).join(
            ...
            SubscriptionPlanFeature.plan_id == plan.id,
            SubscriptionFeature.is_active == True
        ).all()
        
        result.append({
            **SubscriptionPlanResponse.from_orm(plan).dict(),
            "features": [SubscriptionFeatureResponse.from_orm(f) for f in features]
        })
    
    return result

@app.get("/api/public/subscription-plans/{plan_code}")
async def get_public_subscription_plan(
    plan_code: str,
    db: Session = Depends(get_db)
):
    """Get specific plan by code (Public API for external platforms)"""
...
async def get_patients(
    ...
    # If with_phone filter is enabled, return simplified data for WhatsApp
    if with_phone:
        patient_list = []
        for patient in patients:
            # For now, using username as phone identifier
            # In production, you should have a dedicated phone_number field
            patient_list.append({
                "id": patient.id,
                "name": patient.username,
                "phone_number": patient.username,  # Replace with actual phone field when available
                "whatsapp_number": patient.username,  # Replace with actual whatsapp field when available
                "email": patient.email
            })
        return patient_list
...
@app.get("/api/patients/by-phone/{phone_number}")
async def get_patient_by_phone(
    phone_number: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Get patient data by phone number for WhatsApp integration"""
    ...
    # Note: This assumes you have phone_number field in User model
    # If not, you may need to add it or use a different field
    patient = db.query(User).filter(
        User.role == "patient"
    ).filter(
        (User.username.contains(cleaned_phone)) | 
        (User.email.contains(cleaned_phone))
    ).first()
    ...
    patient_data = {
        ...
        "date_of_birth": None,  # Add if available in your model
        "gender": None,  # Add if available in your model
        "appointments": [],
        "health_metrics": {},
        "medications": [],
        ...
        patient_data["appointments"] = [
            {
                ...
                "time": str(appt.appointment_time) if hasattr(appt, 'appointment_time') else None,
                "type": appt.appointment_type if hasattr(appt, 'appointment_type') else None,
                "status": appt.status,
                "therapist": appt.therapist_name if hasattr(appt, 'therapist_name') else None
            }
    ...
    # Fetch health metrics (if you have a health metrics table)
    try:
        # Add health metrics query here if available
        # For now, returning empty dict
        patient_data["health_metrics"] = {
            "weight": None,
            "blood_pressure": None,
            "heart_rate": None
        }
    ...
    # Fetch medications (if you have a medications table)
    try:
        # Add medications query here if available
